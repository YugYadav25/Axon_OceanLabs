...
function parseFile(filePath, repoId, providedModuleName = null) {
  ...
  function pushNode(meta) {
    let uniqueIdName = meta.name;
    if (seenFunctionNames.has(meta.name)) {
      const count = seenFunctionNames.get(meta.name) + 1;
      seenFunctionNames.set(meta.name, count);
      uniqueIdName = `${meta.name}_${count}`;
    } else {
      seenFunctionNames.set(meta.name, 1);
    }

    fileFunctions.push({
      repoId,
      nodeId: `${filePath}::${uniqueIdName}`,
      filePath,
      module: moduleName,
