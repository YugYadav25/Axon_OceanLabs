...
async function answerQuestion(repoId, userId, question) {
  ...
  // Fetch Onboarding Overview for detailed extracted repo data
  let overviewText = '';
  try {
    const overview = await OnboardingOverview.findOne({ user_id: userId, repo_id: repoId }).sort({ createdAt: -1 });
    if (overview && overview.html) {
      overviewText = overview.html.replace(/<[^>]*>?/gm, '\n').replace(/\s+/g, ' ').trim();
    }
  } catch (err) {
  ...
  const contexts = [];
  if (overviewText) {
    // Only include a truncated portion of the overview to save tokens if it's too large, though context limit might handle it
    contexts.push(`Detailed Repository Overview:\n${overviewText.substring(0, 3000)}`);
  }
