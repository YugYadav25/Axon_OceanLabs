...
async function generateOnboardingTasks({ repoId, repoPath: providedRepoPath }) {
...
`;
  ...
  // 2) GitHub Issues (up to 3)
  const issues = await fetchOpenIssues(repoId, 3);
  if (issues.length) {
  ...
  // 3) Code Smells (up to 3)
  const smells = scanCodeSmells(repoPath);
  if (smells.length) {
    smells.slice(0, 3).forEach((s, i) => {
      tasks.push({
        repoId,
        title: `Refactor: ${s.type} in ${path.basename(s.file)}`,
        description: `${s.detail} at line ${s.line}. Suggest opening the file and simplifying the complexity.`,
  ...
  // 4) TODO/FIXME Comments (up to 3)
  const todos = scanTodoComments(repoPath);
  if (todos.length) {
    todos.slice(0, 3).forEach((t, i) => {
      tasks.push({
        repoId,
        title: `Address TODO: ${t.comment}`,
        description: `Found unaddressed TODO/FIXME in ${path.basename(t.file)} at line ${t.line}`,
  ...
  // 5) Smart Fallbacks (Only if we couldn't find any issues/smells/todos)
  if (tasks.filter(t => t.title !== 'Architecture Overview').length === 0) {
    tasks.push({
      repoId,
      title: 'Setup Workflow & Environment',
      description: 'Review package.json scripts and copy .env.example to .env to set up your local development environment.',
    ...
    tasks.push({
      ...
      description: 'Explore the codebase starting from the main entry point to understand data flow.',
