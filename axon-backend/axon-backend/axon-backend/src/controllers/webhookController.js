...
router.post('/github', async (req, res) => {
  try {
    ...
    // --- NEW: PR Review Agent Routing ---
    // If this is a Pull Request event, route it to the AI reviewer and exit.
    if (payload.pull_request) {
      const { handlePRReview } = require('../services/prReviewService');
      const io = req.app.get('io');
      // Fire and forget so we respond to GitHub immediately (prevent 10s timeout)
      handlePRReview(payload, io).catch(err => console.error('PR Review Error:', err));
      return res.status(200).send('PR Webhook received and dispatched for review.');
    }
    // ------------------------------------
    ...
    const repoPath = path.join(require('os').tmpdir(), 'axon', repoRec.repoId);
    ...
    if (!head) return res.status(200).send('No head commit data.');
    
    const modifiedFiles = head.modified || [];
    const addedFiles = head.added || [];
    const removedFiles = head.removed || [];

    // 5) For each removed file, delete all NodeModel docs
    for (const removedPath of removedFiles) {
      ...
      // Remove embeddings for nodes in that file from Mongo
      await CodeEmbedding.deleteMany({
        repo_id: repoRec.repoId,
        type: 'node',
        file_path: { $regex: removedPath, $options: 'i' }
      });
    }
    ...
    for (const relPath of affectedFiles) {
      ...
      // a) Delete any existing NodeModel entries
      await NodeModel.deleteMany({ filePath: absPath });
      // b) Also delete their embeddings in Mongo
      await CodeEmbedding.deleteMany({
        repo_id: repoRec.repoId,
        type: 'node',
        file_path: { $regex: relPath, $options: 'i' }
      });
